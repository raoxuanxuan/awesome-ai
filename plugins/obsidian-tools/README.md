...
首次使用时需要准备用户本机运行时目录和 vault 配置。能自动创建的内容，agent 应优先自动创建；不能自动决定的真实 vault 路径，需要用户提供。

建议 agent 自动执行：

```bash
mkdir -p ~/.obsidian-tools
test -f ~/.obsidian-tools/vaults.json || cp plugins/obsidian-tools/skills/content-to-obsidian/vaults.json.example ~/.obsidian-tools/vaults.json
```
...
把 `root` 改成自己的 Obsidian vault 绝对路径。真实 `vaults.json` 是本机配置，不要提交到 git。
